...
$sql_review_course = "SELECT A.*, B.profile_pic FROM `tbl_review_course` A
                    INNER JOIN `tbl_user_info` B
                    ON A.create_by = B.username
                    WHERE `course` = $course_id $score
                    ORDER BY `created_time` DESC";
$result_review_course = mysqli_query($conn, $sql_review_course);

if (mysqli_num_rows($result_review_course) != 0) {
?>
    <table id="tbl_review" class="tbl_review" width="100%">
        ...
        <tbody>
            <?php
            foreach ($result_review_course as $row) {
            ?>
